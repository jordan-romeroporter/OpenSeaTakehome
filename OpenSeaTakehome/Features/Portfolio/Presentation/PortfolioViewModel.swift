...
final class PortfolioViewModel: ObservableObject {
    ...
    func loadPortfolio() async {
        ...
        async let tokensTask = portfolioService.fetchTokens(for: walletAddress, networks: networks)
        async let nftsTask = portfolioService.fetchNFTContracts(for: walletAddress, networks: networks)
...
enum ValidationError: LocalizedError {
    ...
    var errorDescription: String? {
        NSLocalizedString("error.invalid_address", comment: "Invalid wallet address")
    }
    
    var recoverySuggestion: String? {
        NSLocalizedString("error.invalid_address.recovery", comment: "Please enter a valid Ethereum address (0x...)")
    }
}
