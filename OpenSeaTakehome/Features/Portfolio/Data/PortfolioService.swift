...
protocol PortfolioService: AnyObject {
    func fetchTokens(for address: String, networks: [SupportedNetwork])
        async throws -> [Token]
    func fetchNFTContracts(for address: String, networks: [SupportedNetwork])
        async throws -> [NFTContract]
}
...
final class PortfolioServiceImpl: PortfolioService {
        ...
        async throws -> [Token]
    {
        ...
        let response: TokensResponse = try await networkService.post(
            endpoint,
            body: requestBody
        )
        ...
        async throws -> [NFTContract]
    {
        ...
        let requestBody = NFTContractsRequest(
            address: address,
            networks: networks
        )
        let response: NFTContractsResponse = try await networkService.post(
            endpoint,
            body: requestBody
        )
